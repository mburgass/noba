...
#FMSY0
  ...
  df<- fmsy0_data %>% dplyr::filter(ID<30)
  rm(fmsy0_data)
  index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
  index_vector[1:155] = TRUE #change to number of rows as above
...
#FMSY05  
  df<- fmsy05_data %>% dplyr::filter(ID<30)
  rm(fmsy05_data)
  index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
  index_vector[1:155] = TRUE #change to number of rows as above
...
#FMSY1  
  df<- fmsy1_data %>% dplyr::filter(ID<30)
  rm(fmsy1_data)
  index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
  index_vector[1:155] = TRUE #change to number of rows as above
...
#FMSY2
  df<- fmsy2_data %>% dplyr::filter(ID<30)
  rm(fmsy2_data)
  index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
  index_vector[1:155] = TRUE #change to number of rows as above
...
#MPA10
df<- mpa10_data %>% dplyr::filter(ID<30)
rm(mpa10_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#MPA25
df<- mpa25_data %>% dplyr::filter(ID<30)
rm(mpa25_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#MPA50
df<- mpa50_data %>% dplyr::filter(ID<30)
rm(mpa50_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#cc2
df<- cc2_data %>% dplyr::filter(ID<30)
rm(cc2_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#CC3
df<- cc3_data %>% dplyr::filter(ID<30)
rm(cc3_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#oa005
df<- oa005_data %>% dplyr::filter(ID<30)
rm(oa005_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
...
#oa01
df<- oa01_data %>% dplyr::filter(ID<30)
rm(oa01_data)
index_vector = rep(FALSE, nrow(df)) #next row reliant on how many rows - need to change if not working
index_vector[1:155] = TRUE #change to number of rows as above
