...
read.csv("chapter_4/biomass/fmsy1_biomass.csv", check.names=F)  %>% gather("Binomial", "biomass", 3:55)%>%
  ...
  select(Year, total_biomass_surveyed) %>% unique() %>% mutate(scenario="fmsy1") ->fmsy1_data
...
read.csv("chapter_4/biomass/fmsy06_biomass.csv", check.names=F) %>% gather("Binomial", "biomass", 3:55) %>%
  filter(Binomial %in% c('CAP','PEL', 'REO', 'GRH', 'MAC', 'HAD','SAI', 'RED','BWH', 'SSH', 'NCO', 'PCO', 'PWN', 'KCR', 'SCR', 'ZL', 'ZM', 'ZS'))%>%
  filter(Year>2014) %>% group_by(Year) %>% mutate(total_biomass_surveyed= mean(biomass)) %>% ungroup() %>%
  select(Year, total_biomass_surveyed) %>% unique() %>% mutate(scenario="fmsy06") -> fmsy06_data
...
surveyed_biomass<- rbind(fmsy0_data, fmsy06_data, fmsy08_data, fmsy1_data, fmsy11_data)
...
read.csv("chapter_4/catch/fmsy_1_catch.csv") %>%select(Year, BWH, MAC, PWN, SSH, REO, GRH, HAD, SAI, RED, NCO, CAP) %>% 
  ...
  unique()%>% mutate(scenario="fmsy1") %>% filter(Year>2014) -> fmsy1_catch  
...
read.csv("chapter_4/catch/fmsy_0.6_catch.csv") %>%select(Year, BWH, MAC, PWN, SSH, REO, GRH, HAD, SAI, RED, NCO, CAP) %>% 
  gather("species", "catch", 2:8) %>% group_by(Year) %>%
  mutate(total_catch=sum(catch)) %>% ungroup() %>% select(Year, total_catch) %>%
  unique()%>% mutate(scenario="fmsy06") %>% filter(Year>2014) -> fmsy06_catch 
...
read.csv("chapter_4/biomass/fmsy06_biomass.csv", check.names=F)  %>% gather("Binomial", "biomass", 3:55)%>%
  filter(Binomial %in% c('REO', 'GRH', 'HAD', 'SAI', 'RED', 'NCO', 'BWH', 'MAC', 'PWN', 'SSH', 'CAP'))%>%
  filter(Year>2014) %>% group_by(Year) %>% mutate(total_biomass_landings= sum(biomass)) %>% ungroup() %>%
  select(Year, total_biomass_landings) %>% unique() %>% mutate(scenario="fmsy06") ->fmsy06_data
...
fish_pressure_catch<- rbind(fmsy06_catch, fmsy08_catch, fmsy1_catch, fmsy11_catch)
fish_pressure_biomass<- rbind(fmsy06_data, fmsy08_data, fmsy1_data, fmsy11_data)
...
read.csv("chapter_4/biomass/fmsy06_biomass.csv", check.names=F) %>% gather("Binomial", "biomass", 3:55) %>%
  filter(Binomial %in% c('DEO','PEL', 'REO','DEL', 'LRD', 'SSK', 'GRH','HAD', 'SAI', 'NCO'))%>%
  filter(Year>2014) %>% group_by(Year) %>% mutate(total_biomass_pred= mean(biomass)) %>% ungroup() %>%
  select(Year, total_biomass_pred) %>% unique() %>% mutate(scenario="fmsy06") ->fmsy06_pred
...
pred_fish<- rbind(fmsy0_pred, fmsy06_pred, fmsy08_pred, fmsy1_pred, fmsy11_pred)
pred_fish$Year<- as.integer(pred_fish$Year)
pred_surveyed<- filter(surveyed_biomass, scenario %in% c('fmsy0', 'fmsy06', 'fmsy08', 'fmsy1', 'fmsy11'))
...
read.csv("chapter_4/biomass/fmsy06_biomass.csv", check.names=F) %>% gather("Binomial", "biomass", 3:55) %>%
  ...
  mutate(biomass_year= sum(biomass)) %>% ungroup() %>% group_by(Year) %>%
  mutate(mean_life_span= max_age_biomass_year/biomass_year) %>% select(Year, mean_life_span) %>% 
  unique()%>% mutate(scenario="fmsy06")->life_span_fmsy06
...
mean_lifespan<- rbind(life_span_fmsy0, life_span_fmsy06, life_span_fmsy08, life_span_fmsy1, life_span_fmsy11)
...
read.csv("chapter_4/catch/fmsy_0.6_catch.csv") %>%select(Year,REO, GRH, HAD, SAI, RED, NCO, BWH, MAC, PWN, SSH, CAP) %>% 
  ...
  mutate(catch_year=sum(catch)) %>% mutate(tl_catch_year= sum(tl_catch)) %>%
  mutate(trophic_level_landings= tl_catch_year/catch_year) %>% ungroup() %>% select(Year, trophic_level_landings) %>%
  unique() %>% mutate(scenario='fmsy06')-> tl_landings_fmsy06
...
tl_landings<- rbind(tl_landings_fmsy06, tl_landings_fmsy08, tl_landings_fmsy1, tl_landings_fmsy11)
...
read.csv("chapter_4/biomass/fmsy06_biomass.csv", check.names=F) %>% gather("Binomial", "biomass", 3:55) %>%
  ...
  mutate(biomass_year=sum(biomass)) %>% mutate(TL_biomass_year= sum(tl_community)) %>%
  mutate(TL_all= TL_biomass_year/biomass_year) %>% ungroup() %>% select(Year, TL_all) %>%
  unique() %>% mutate(scenario='fmsy06')-> TL_community_fmsy06
...
tl_community<- rbind(TL_community_fmsy0, TL_community_fmsy06, TL_community_fmsy08, TL_community_fmsy1, TL_community_fmsy11) #%>% filter(year>1983)
...
a<- ggplot(surveyed_biomass, aes(Year, total_biomass_surveyed)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")
b<- ggplot(fish_pressure, aes(Year, inversepressure)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")
c<- ggplot(pred_fish, aes(Year, proppred)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")
d<- ggplot(mean_lifespan, aes(Year, mean_life_span)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")
e<- ggplot(tl_landings, aes(Year, trophic_level_landings)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")
f<- ggplot(tl_community, aes(Year, TL_all)) +geom_line(aes(colour=scenario), lwd=1.5)+ theme(legend.position="none")

legend_biomass<-ggplot(tl_community, aes(Year, TL_all)) +geom_line(aes(colour=scenario), lwd=1.5)
legend_catch<-ggplot(fish_pressure, aes(Year, inversepressure)) +geom_line(aes(colour=scenario), lwd=1.5)

legend_biomass<- get_legend(legend_biomass)
legend_catch<- get_legend(legend_catch)

prow<- plot_grid(a, c, d, f, labels = c("Surveyed Biomass", "Proportion Predators", "Mean Lifespan", "Trophic Level Community"), align = "v", ncol=2)
plot_grid(prow, legend_biomass, rel_widths = c(3, .3), label_size = 8)


prow2<- plot_grid(b, e, labels = c("Inverse Presssure", "Trophic Level Landings"), align = "v", ncol=1)
