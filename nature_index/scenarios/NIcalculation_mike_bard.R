...
# Read input data
fmsy0_natureindex<- read.csv("nature_index/scenarios/fmsy0_natureindex.csv")
fmsy1_natureindex<- read.csv("nature_index/scenarios/fmsy1_natureindex.csv")
fmsy2_natureindex<- read.csv("nature_index/scenarios/fmsy2_natureindex.csv")
virgin_biomass_natureindex<- read.csv("nature_index/scenarios/virgin_biomass_natureindex.csv")
ni_indicators_noba<- read.csv("nature_index/scenarios/ni_indicators_noba.csv")
...
##HERE IT IS MIKE!
##You can change the weighing of key indicators as a group by changing w in the
##line of code below. w should be between 0 (zero weight, which means that key indicators
##in practise are excluded from the calculation) and 1 (which means that only
##key indicators are included). I am not sure whether the code will function for
##these two extreme values - but I think so - you can try.
##Changing w still keep all trophic levels the same weight.See the function
##assign.trophic.weights above.
##
##If you in the calculations want to treat key-indicators, as a group, 
##in the same way as the trophic
##levels, the easiest way is to set the key.indicator.code in the functions calls 
##below to a value that does not occur in the vector functional.group.
##
##If you want to avoid the concept of key indicators all together, 
##and instead treat those indicators as any other, that is explained above,
##i.e skip code line 290.
##
##If you want to give unequal weighting of the trophic levels, I guess you
##have to change the functions above that calculate trophic weights.
##
##Hope this was helpful, B?rd.
